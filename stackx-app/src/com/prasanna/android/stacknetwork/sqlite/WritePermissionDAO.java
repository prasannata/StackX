...
public class WritePermissionDAO extends AbstractBaseDao {
  ...
  public static final class WritePermissionTable {
    ...
    protected static final String CREATE_TABLE = "create table " + TABLE_NAME + "(" + COLUMN_ID
        ...
        + " integer not null, " + COLUMN_WAIT_TIME + " integer not null, " + COLUMN_OBJECT_TYPE + " text not null, "
        + COLUMN_SITE + " text not null, " + COLUMN_SITE_URL + " text not null);";
  }
  ...
  private WritePermission getPermission(Cursor cursor) {
    ...
    permission.maxDailyActions = cursor.getInt(cursor.getColumnIndex(WritePermissionTable.COLUMN_MAX_DAILY_ACTIONS));
    permission.minSecondsBetweenActions = cursor.getInt(cursor.getColumnIndex(WritePermissionTable.COLUMN_WAIT_TIME));
    permission.objectType =
        ObjectType.getEnum(cursor.getString(cursor.getColumnIndex(WritePermissionTable.COLUMN_OBJECT_TYPE)));
    return permission;
  }
